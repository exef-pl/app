#!/usr/bin/env python3
"""
Test spójności ID dla wszystkich formatów - użycie standardowych danych
"""
...
def main():
    print("=" * 80)
    print("TEST SPÓJNOŚCI ID - WSZYSTKIE FORMATY (STANDARDOWE DANE)")
    print("=" * 80)
        ...
        for file_path in files:
            try:
                ...
                if doc_id:
                    ...
                    print(f"  📄 {file_path.name:<25} ({file_path.suffix:<5}) [{file_path.stat().st_size:>6}B] -> {doc_id}")
                else:
    ...
    print(f"\n{'-' * 80}")
    print("TEST 10-KROTNEJ DETERMINISTYCZNOŚCI DLA KAŻDEGO FORMATU")
    print(f"{'-' * 80}")
    ...
    for folder, folder_name in folders.items():
        ...
        data = standard_data[folder]
        sample_file = results[folder]['files'][0]
        sample_ext = sample_file['extension']
        
        print(f"\nTestowanie {folder_name} ({sample_ext}):")
