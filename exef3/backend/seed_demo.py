...
def seed():
    ...
    templates = [
        ...
        ProjectTemplate(
            id=uid(), code="korespondencja",
            name="Korespondencja",
            description="Rejestr korespondencji przychodzącej i wychodzącej. Pisma urzędowe, reklamacje, wezwania.",
            project_type=ProjectType.KORESPONDENCJA,
            task_recurrence=TaskRecurrence.MONTHLY,
            task_name_template="Korespondencja {month_name} {year}",
            task_icon="✉️", deadline_day=5,
            default_icon="✉️", default_color="#6366f1",
            default_categories=["Pismo urzędowe", "Reklamacja", "Wezwanie", "Zawiadomienie", "Zaproszenie", "Oferta", "Inne"],
            is_system=True,
        ),
        ProjectTemplate(
            id=uid(), code="zamowienia",
            name="Zamówienia",
            description="Zarządzanie zamówieniami zakupowymi i sprzedażowymi. Śledzenie wartości, statusów realizacji.",
            project_type=ProjectType.ZAMOWIENIA,
            task_recurrence=TaskRecurrence.MONTHLY,
            task_name_template="Zamówienia {month_name} {year}",
            task_icon="🛒", deadline_day=5,
            default_icon="🛒", default_color="#f97316",
            default_categories=["Zakup", "Sprzedaż", "Zwrot", "Reklamacja", "Inne"],
            is_system=True,
        ),
        ProjectTemplate(
            id=uid(), code="protokoly",
            name="Protokoły",
            description="Protokoły odbioru, zebrań, kontroli. Rejestr ustaleń i decyzji.",
            project_type=ProjectType.PROTOKOLY,
            task_recurrence=TaskRecurrence.ONCE,
            task_name_template="Protokoły {year}",
            task_icon="📋", deadline_day=30,
            default_icon="📋", default_color="#14b8a6",
            default_categories=["Odbiór", "Zebranie", "Kontrola", "Inwentaryzacja", "Inne"],
            is_system=True,
        ),
        ProjectTemplate(
            id=uid(), code="polisy_ubezpieczenia",
            name="Polisy / Ubezpieczenia",
            description="Zarządzanie polisami ubezpieczeniowymi. Śledzenie terminów, składek i zakresów.",
            project_type=ProjectType.POLISY,
            task_recurrence=TaskRecurrence.ONCE,
            task_name_template="Polisy {year}",
            task_icon="🛡️", deadline_day=30,
            default_icon="🛡️", default_color="#ec4899",
            default_categories=["OC", "AC", "Majątkowe", "Zdrowotne", "Grupowe", "D&O", "Inne"],
            is_system=True,
        ),
        ProjectTemplate(
            id=uid(), code="wnioski",
            name="Wnioski",
            description="Rejestr wniosków: urlopowych, zakupowych, reklamacyjnych i innych.",
            project_type=ProjectType.WNIOSKI,
            task_recurrence=TaskRecurrence.MONTHLY,
            task_name_template="Wnioski {month_name} {year}",
            task_icon="📨", deadline_day=5,
            default_icon="📨", default_color="#a855f7",
            default_categories=["Urlop", "Zakup", "Reklamacja", "Zmiana danych", "Delegacja", "Szkolenie", "Inne"],
            is_system=True,
        ),
        ProjectTemplate(
            id=uid(), code="nieruchomosci",
            name="Nieruchomości",
            description="Dokumenty nieruchomości: umowy najmu, akty notarialne, protokoły zdawczo-odbiorcze.",
            project_type=ProjectType.NIERUCHOMOSCI,
            task_recurrence=TaskRecurrence.ONCE,
            task_name_template="Nieruchomości {year}",
            task_icon="🏠", deadline_day=30,
            default_icon="🏠", default_color="#84cc16",
            default_categories=["Najem", "Sprzedaż", "Akt notarialny", "Protokół zdawczo-odbiorczy", "Inne"],
            is_system=True,
        ),
    ]
    for t in templates:
