    ...
    <>
      ...
      <div style={{ borderBottom: `1px solid ${COLORS.border}` }}>
        ...
        {docsTotal > 0 && (
          <div style={{
            ...
            <span>{sl.new}: <strong style={{ color: COLORS.warning }}>{docsNew}</strong></span>
            <span>{sl.described}: <strong style={{ color: COLORS.primary }}>{docsDescribed}</strong></span>
            <span>{sl.exported}: <strong style={{ color: COLORS.success }}>{docsExported}</strong></span>
            {tc.showExportWarning && notExported > 0 && (
      ...
      {docsTotal > 0 && (
        <div style={{
          ...
          <input
            type="text" placeholder={tc.searchPlaceholder}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            ...
            <tbody>
              {(() => {
                ...
                const idCounts = {};
                if (tc.showDuplicates) {
                  documents.forEach(d => { if (d.doc_id) idCounts[d.doc_id] = (idCounts[d.doc_id] || 0) + 1; });
                }
                return sorted.map(doc => {
                const status = STATUS_CONFIG[doc.status];
                const isDup = tc.showDuplicates && doc.doc_id && idCounts[doc.doc_id] > 1;
                      ...
                      return (
                        <td key={col.key} style={{ padding: '12px 8px' }}>
                          {doc.metadata?.category ? (
                            ...
                            <span style={{
                              color: tc.emptyCategoryStyle === 'warning' ? COLORS.warning : COLORS.textMuted,
                              fontSize: '11px', fontStyle: tc.emptyCategoryStyle === 'muted' ? 'italic' : 'normal',
                            }}>{tc.emptyCategory}</span>
                          )}
                        </td>
                      );
                    case 'tags':
